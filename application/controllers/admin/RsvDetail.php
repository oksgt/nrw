...
class RsvDetail extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('Spam_rsv_detail_model');
    ...
    public function index($id)
    {
        ...
        // echo "<pre>";
        // print_r($data); die;
        $this->load->view('admin/rsvdetail_view', $data);
    }

    public function ajax_list($node_id)
    {
        $list = $this->Spam_rsv_detail_model->get_datatables($node_id);
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $r) {
            $no++;
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->Spam_rsv_detail_model->count_all($node_id),
            "recordsFiltered" => $this->Spam_rsv_detail_model->count_filtered($node_id),
            "data" => $data,
        );
    ...
    public function validation()
    {
        ...
        if ($this->form_validation->run()) {
            ...
            $array = array(
                'error' => true,
                'input_debit_input_error_detail'        => form_error('input_debit_input', '<b class="fa fa-exclamation-triangle"></b> ', ' '),
    ...
    public function insert()
    {
        ...
        $inserted = $this->Spam_rsv_detail_model->save($data);
        if ($inserted) {
    ...
    function detail($id = "")
    {
        $data = $this->Spam_rsv_detail_model->get_by(array('id' => $id))->row_array();
    ...
    public function update()
    {
        ...
        $existing_data = $this->Spam_rsv_detail_model->get_by(array('ID' => $this->input->post('id', true)))->row_array();
        ...
        if (!empty($edited_data)) { //if there is any edited data
            $updated = $this->Spam_rsv_detail_model->update($object, array('ID' => $this->input->post('id', true)));
            if ($updated) {
                $result = array('status' => true);
            } else {
                $result = array('status' => false);
    ...
    public function delete($id)
    {
        ...
        $where = array('id' => $id);
        $affected_row = $this->Spam_rsv_detail_model->update($object, $where);
        $result = array('status' => true);
        echo json_encode($result);
    }
