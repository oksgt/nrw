          ...
          <div class="modal-dialog">
            <div class="modal-content">
              ...
              <div class="modal-body">
                <form id="form-step" method="post" onsubmit="return false;">
                  <input type="hidden" name="id">
                  <div class="form-group">
                    <label for="input_step">Nama Step</label>
                    <input type="text" class="form-control" id="input_step" name="input_step">
                    <small id="input_step_error_detail" class="form-text text-danger"></small>
                  </div>
              </div>
    ...
    <script type="text/javascript">
      ...
      $(document).ready(function() {
        ...
        table = $('#table').DataTable({
          ...
          "ajax": {
            "url": "<?php echo site_url('admin/step/ajax_list') ?>",
            "type": "POST"
          },
          "columnDefs": [{
            "targets": [-1],
      ...
      function form_validation() {
        $('#form-step').on('submit', function(event) {
          ...
          var input_list = ['input_step'];
          var input_list_error = ['input_step_error_detail'];

          $.ajax({
            url: "<?php echo site_url('admin/step/validation') ?>",
            method: 'post',
            data: $(this).serialize(),
            dataType: 'json',
            beforeSend: function() {
      ...
      function save() {
        var url;
        if (save_method == "add") {
          url = "<?php echo site_url('admin/step/') ?>" + "insert/";
        } else {
          url = "<?php echo site_url('admin/step/') ?>" + "update/";
        }
      ...
      function detail(id) {
        reset_validation();
        $.ajax({
          url: "<?php echo site_url('admin/step/') ?>" + "detail/" + id,
          method: 'GET',
          dataType: 'json',
          success: function(data) {
      ...
      function hapus_data(id) {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: "<?php echo site_url('admin/step/') ?>" + "delete/" + id,
              method: "GET",
              dataType: 'json',
              success: function(data) {
                if (data.status) {
